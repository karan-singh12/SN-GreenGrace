...
export default function ProductDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-transparent relative pb-20 pt-24 md:pt-32 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
      <SparklesBackground />
      <Navbar />
      ...
      <div className="mb-6 flex justify-start">
        <Link 
          href="/" 
          className="flex items-center gap-1 text-primary hover:opacity-80 transition-opacity font-bold text-xs"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 bg-white/70 backdrop-blur-xl rounded-3xl p-6 md:p-10 border border-emerald-500/10 shadow-sm mb-12">
        {/* Left Column: Image wrapper */}
        <div className="flex flex-col items-center justify-center p-6 bg-white border border-emerald-500/10 rounded-2xl h-80 md:h-[400px] overflow-hidden relative shadow-inner group">
          {product.isOrganic && (
            <span className="absolute top-4 left-4 z-10 px-2.5 py-1 rounded bg-[#085427]/10 text-[#085427] font-bold text-[10px] uppercase tracking-wider border border-[#085427]/15">
              Organic
            </span>
          )}
          <img 
        ...
        <div className="flex flex-col justify-between py-2">
          <div className="space-y-4">
            <div>
              <span className="text-[10px] text-primary font-bold uppercase tracking-wider bg-primary/10 px-2.5 py-1 rounded">
                {product.category}
              </span>
              <h1 className="font-extrabold text-2xl md:text-3xl text-on-surface mt-2">{product.name}</h1>
            ...
            <div className="py-2.5 border-t border-b border-emerald-500/10 flex justify-between items-center">
              <div>
                <span className="text-[10px] text-outline font-bold uppercase block tracking-wider">Price</span>
              ...
              <div className="text-right">
                ...
                <span className="text-xs font-bold text-emerald-700 flex items-center gap-0.5 justify-end">
          ...
          <div className="mt-8 pt-4 border-t border-emerald-500/10">
            {quantityInCart > 0 ? (
              <div className="flex flex-col gap-2.5">
                <span className="text-[10px] text-primary font-extrabold uppercase tracking-wide block">
                  Already in Basket
                </span>
                <div className="flex items-center gap-3">
                  <div className="flex items-center bg-primary/10 rounded-xl border border-primary/20 p-1 shrink-0 shadow-sm">
                    <button
                      onClick={() => updateQuantity(product.id, quantityInCart - 1)}
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-primary font-black hover:bg-primary/25 transition-colors cursor-pointer"
                    >
                      -
                    </button>
                    <span className="w-10 text-center font-bold text-xs md:text-sm text-primary select-none">
                      {quantityInCart}
                    </span>
                    <button
                      onClick={() => updateQuantity(product.id, quantityInCart + 1)}
                  ...
                  <div className="text-xs text-on-surface-variant font-bold">
                    Subtotal: <strong className="text-primary">₹{(product.price * quantityInCart).toFixed(2)}</strong>
                  </div>
                </div>
              </div>
            ) : (
              <button
                ...
                className="w-full sm:w-auto px-8 py-3.5 bg-primary text-on-primary hover:bg-primary-hover rounded-xl font-bold text-xs md:text-sm shadow-md transition-all active:scale-95 flex items-center justify-center gap-2"
              >
      ...
      {recipes.length > 0 && (
        <section className="flex flex-col gap-6 mt-12">
          <div className="flex items-center gap-2 border-b border-emerald-500/10 pb-3">
            ...
            <div>
              <h2 className="font-extrabold text-base md:text-xl text-on-background">AI Cooking Suggested Recipes</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {recipes.map(recipe => {
              ...
              return (
                <div 
                  key={recipe.id}
                  className="bg-white/70 backdrop-blur-md border border-emerald-500/10 rounded-3xl p-5 md:p-6 shadow-sm flex flex-col justify-between gap-4"
                >
                  <div className="flex gap-4">
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-white overflow-hidden shrink-0 flex items-center justify-center border border-emerald-500/10">
                      <img src={recipe.image} alt={recipe.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="font-extrabold text-sm md:text-base text-on-surface truncate">{recipe.name}</h3>
                  ...
                  <div className="border-t border-emerald-500/10 pt-3 flex flex-col gap-3">
                    <div>
                    ...
                    <div className="flex justify-between items-center mt-1.5 flex-wrap gap-2">
                      <div className="text-xs font-semibold">
                        {missingIngredients.length > 0 ? (
                          <span className="text-on-surface-variant font-medium">
                            Missing: <strong className="text-primary">{missingIngredients.length} items</strong>
                          </span>
                        ) : (
                      ...
                      {missingIngredients.length > 0 && (
                        <button
                          ...
                          className="px-3.5 py-2 bg-primary text-on-primary rounded-xl font-bold text-[11px] hover:bg-primary-hover shadow-sm transition-all active:scale-95 flex items-center gap-1"
                        >
